...
extern uint8_t mode, temp_max, humi_max, power_flag, time_set_flag;
...
void ESP8266_Init(void)
{
  ...
  Delay_ms(500);
	...
	Delay_ms(500);
	while(ESP8266_SendCmd("AT+CWSAP=\"GDP001\",\"12345678\",5,3\r\n", "OK", 100) == 1);		/* 建立服务器 */
	Delay_ms(500);
	...
	while(timeout--)
	{
		if((ESP8266_WaitRecive() == REV_OK) && (strstr((char *)ESP8266_BUF, "IPD") != NULL))										//如果接收完成
		{
			...
			if (mode == 1)
			{
				...
				if (value != RXERROR)
					temp_max = value;
				value = Received_Data_Processing("humi_max", strlen("humi_max"));
				if (value != RXERROR)
					humi_max = value;
				...
				{
					time_set_flag = 1;
					set_time.minute = value;
				}
				value = Received_Data_Processing("sec", strlen("sec"));
				if (value != RXERROR)
				{
					time_set_flag = 1;
					set_time.second = value;
				}
				value = Received_Data_Processing("power_state", strlen("power_state"));
				if (value != RXERROR)
				{
					power_flag = value;
				}
